/**
 * OrdersPage — single aggregate API for src/pages/orders/OrdersPage.tsx
 * GET /api/orders/orders-page
 */
...
const getMyOrders = async (userId) => {
  const { recordset: rows } = await query(
    orderQuery + ' WHERE o.user_id = ? ORDER BY o.created_at DESC',
    [userId]
  );
...
router.get('/orders-page', authenticate, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { orders, shops } });
  } catch (err) {
