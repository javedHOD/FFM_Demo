/**
 * AdminSalesPage — single aggregate API for src/pages/admin/AdminSalesPage.tsx
 * GET /api/sales/admin-sales-page?from=YYYY-MM-DD&to=YYYY-MM-DD
 */
const express = require('express');
const router = express.Router();
const { query } = require('../db/connection');
const { authenticate } = require('../middleware/auth');

const salesQuery = `
  SELECT se.*, u.full_name as user_name, s.shop_name
  FROM sales_entries se
  LEFT JOIN users u ON se.user_id = u.id
  LEFT JOIN shops s ON se.shop_id = s.id
`;

const formatSales = (row) => ({
  id: row.id,
  userId: row.user_id,
  userName: row.user_name,
  shopId: row.shop_id,
  shopName: row.shop_name,
  productName: row.product_name,
  quantity: row.quantity,
  amount: parseFloat(row.amount),
  remarks: row.remarks,
  createdAt: row.created_at,
});

const parseDateParam = (value) => {
  if (!value || typeof value !== 'string') return null;
  return /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : null;
};

const buildDateFilter = (queryParams) => {
  const from = parseDateParam(queryParams.from);
  const to = parseDateParam(queryParams.to);
  const conditions = [];
  const params = [];

  if (from) {
    conditions.push('CAST(se.created_at AS DATE) >= CAST(? AS DATE)');
    params.push(from);
  }
  if (to) {
    conditions.push('CAST(se.created_at AS DATE) <= CAST(? AS DATE)');
    params.push(to);
  }

  return { conditions, params };
};

const dateWhereClause = (conditions) =>
  conditions.length ? ` WHERE ${conditions.join(' AND ')}` : '';

const getSales = async (conditions, params) => {
  const { recordset: rows } = await query(
    salesQuery + dateWhereClause(conditions) + ' ORDER BY se.created_at DESC',
    params
  );
  return rows.map(formatSales);
};

const getSummaryByUser = async (conditions, params) => {
  const { recordset: rows } = await query(`
    SELECT u.id as user_id, u.full_name as user_name,
           SUM(se.amount) as total_amount, COUNT(*) as total_orders
    FROM sales_entries se
    LEFT JOIN users u ON se.user_id = u.id
    ${dateWhereClause(conditions)}
    GROUP BY se.user_id, u.id, u.full_name
    ORDER BY total_amount DESC
  `, params);
  return rows.map((r) => ({
    userId: r.user_id,
    userName: r.user_name,
    totalAmount: parseFloat(r.total_amount || 0),
    totalOrders: r.total_orders,
  }));
};

// GET /api/sales/admin-sales-page
router.get('/admin-sales-page', authenticate, async (req, res) => {
  try {
    const { conditions, params } = buildDateFilter(req.query);
    const [sales, salesByUser] = await Promise.all([
      getSales(conditions, params),
      getSummaryByUser(conditions, params),
    ]);

    res.json({
      success: true,
      data: { sales, salesByUser },
    });
  } catch (err) {
    console.error('admin-sales-page error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

module.exports = router;
